---
title: "Applications of the Delta Method in Pharmacometrics"
author: "Samer Mouksassi"
date: "2025-12-01"
categories: [Methods, visualization, uncertainty]
image: "qivshdi.png"
format:
 html:
  code-fold: true
...
$$CL = \color{blue}{POPCL}\times\left(\frac {\color{green}{Weight}} {70}\right)^\color{blue}{dWTdCL}\times\left(\frac{\color{green}{Age}}{\color{green}{Age} + \color{blue}{Age50}}\right)\times e^{\color{red}\eta\color{red}{CL}}$$

In this example the parameter clearance (CL) has two covariates effects:

-   A continuous covariate power model of body weight (Weight) with a reference value of 70 (kg)

-   A continuous covariate emax/hill maturation model as a function of Age.

The last term denotes the individual deviations from the population (random effects) which assumes that the population CL is log normally distributed.

Common software will estimate the fixed and random effects variances and provide the associated standard errors. For example we would get POPCL = 5 ± 0.25 L/h and the Sex effect is 0.18 ± 0.05 resulting in a Clearance of 5 × exp(0.18) = 5.99 L/h in men. What would be its standard error?

Also, we often want to report the [CV%](https://www.page-meeting.org/wp-content/uploads/pdf_assets/4964-Elassaiss-Schaap%20-%20Equations%20variability%20reporting%20PK-PD%20-%20Final.pdf) of the log normal distribution expressed as:

$$CV(\%)=\sqrt{\left(e^{\omega^2}-1\right)}$$ Again, what would be the standard error of the CV%?

The delta method is a statistical technique to approximate the standard error of a function of an estimator, such as a ratio or the log odds ratio or the inverse logit of a constrained parameter, using a first-order Taylor series expansion. It's especially useful when the function involves more than one parameter and/or the equation is complex such as there is no straightforward analytically computed solution. It relies on the original variable(s), the function form and the partial derivative of the function with respect to the parameters and the variance covariance matrix of the involved parameters. It has been used in multiple fields, including pharmacometrics, that the delta method matches the gold standard (simulation with uncertainty) even with [longitudinal model based meta-analysis (MBMA) nonlinear models](https://www.page-meeting.org/wp-content/uploads/pdf_assets/6769-Poster_PAGE_2012_MegBennetts.pdf). Also, in this important paper from the FDA entitled [Clarification on Precision Criteria to Derive Sample Size When Designing Pediatric Pharmacokinetic Studies](https://accp1.onlinelibrary.wiley.com/doi/10.1177/0091270011422812) R code to compute the delta method for derived variables was provided in **APPENDIX 3. R code for delta method to estimate standard error for a derived variable:**

$$log(CL) = \color{blue}{3.7421} + \color{blue}{1.0078}\times\left( \frac { \color{green}{Weight}} {70}\right)+log\left(\color{green}{Age}/(\color{green}{Age}+4.8422)\right)$$

```{r}
#| code-fold: false
# Assume covariate model CL=theta1*(wt/70)** theta2*age/(age+theta3) and
# Log of CL was modeled as LCL=thetap1+theta2*(wt/70)+log(age/(age+theta3))
...
The code above works, but it is easier to use the R package [msm](https://cran.r-project.org/web/packages/msm/index.html) which has a streamlined general `deltamethod` function of which I have a copy in my own package [coveffectsplot](https://smouksassi.github.io/coveffectsplot/reference/deltamethod.html). The only difference in mine is explicitly adding the evaluation environment which was necessary for the function to properly work in an R Shiny App. The `deltamethod` function source code:
...
In this post I have shown several examples/applications of the delta method to compute confidence intervals for and important model statistic that is a function of one or more model parameters. I have also showed how to compute intervals around random effects distributions via simulations arguing that a single stat such as CV% is inappropriate for non-symmetrical and/or multi-modal distributions using an inverse logit transformation of bioavailibility (F) as an example.
