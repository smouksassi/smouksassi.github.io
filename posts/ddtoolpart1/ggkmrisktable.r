...
ggkmrisktable <- function(data = lung_long, # long format filter to Endpoint of choice
  ...
  if(all(exposure_metrics%in% c("none",""))) {
    ...
    data.long <- data |>
      tidyr::gather(expname,expvalue,none, factor_key = TRUE) |> 
      dplyr::group_by(expname,!!endpoint,colorinputvar)    
  }
  ...
  #         arrange(expname,exptile,AGE_CATEGORY))# xxx
  # 
  # print(data.long %>% 
  #         dplyr::group_by(expname,AGE_CATEGORY)|>
  #         dplyr::summarize(med=median(expvalue)))
    ...
    plotkm1 <-  plotkm000 +
      ggplot2::geom_text(data=risktabledatag,
                         ggplot2::aes(x=time,label=value,y=keynumeric,time=NULL,status=NULL,
                                      fontface = ifelse(exptile=="M1",1,2)),
                         show.legend = FALSE,
  ...
  if(show_exptile_values){
    ...
    if(show_exptile_values_order=="reverse") {
      plotkm1mtexp <- plotkm1mt +
        ggrepel::geom_text_repel(data = data.long.quantiles |>
                             dplyr::mutate(none = "(all)",
                                           "{timevar}" := NA,
                                           "{statusvar}" := NA), 
                           ggplot2::aes(x = exptile_values_pos_x,
                                        y = km_median_table_y_multiplier*rev(as.numeric(
                                          as.factor(paste(!!colorinputvar,exptile)))),
      ...
      plotkm1mtexp <- plotkm1mt +
        ggrepel::geom_text_repel(data = data.long.quantiles |>
                             dplyr::mutate(none = "(all)",
                                           ...
                                           "{statusvar}" := NA), 
                           ggplot2::aes(x = exptile_values_pos_x,
                                        y = km_median_table_y_multiplier*(as.numeric(
                                          as.factor(paste(!!colorinputvar,exptile)))),
                                        label = paste0(exptile, ": ",exprange
                                        )), 
                           hjust = exptile_values_pos_x_hjust,
  ...
  if(return_list){
    ...
    if(km_logrank_pvalue) {
      pf <- list(pf,logrank_test_by_endpoint)
    }
